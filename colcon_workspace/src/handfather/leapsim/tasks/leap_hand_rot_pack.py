...
class LeapHandRot(VecTaskRot):
    def __init__(self, cfg, rl_device, sim_device, graphics_device_id, headless, virtual_screen_capture=None, force_render=None):
        ...
        self.joint_log_enabled = True
        if "joint_logging" in self.cfg["env"] and self.cfg["env"]["joint_logging"]["enabled"]:
            self.joint_log_enabled = True
            self.joint_log = []
            
            log_freq_hz = self.cfg["env"]["joint_logging"].get("frequency_hz", 30)
            self.log_interval = max(1, int(1.0 / (log_freq_hz * self.control_dt)))

            self.joint_log_dir = self.cfg["env"]["joint_logging"].get("output_dir", "logs/inference_data")
            self.joint_log_prefix = self.cfg["env"]["joint_logging"].get("file_prefix", "trajectory")
            self.episode_log_counter = 0

            if not os.path.exists(self.joint_log_dir):
                os.makedirs(self.joint_log_dir)
            
            print("\n" + "="*50)
            print(f"[日志记录器] 已启用！")
            print(f"  - 数据将保存在: '{self.joint_log_dir}'")
            print(f"  - 文件名前缀: '{self.joint_log_prefix}'")
            print(f"  - 记录频率: {log_freq_hz} Hz (每 {self.log_interval} 步记录一次)")
            print("="*50 + "\n")
    def _save_joint_log(self, filename):
        """将收集到的关节数据保存到 .npy 文件。"""
        if not self.joint_log: 
            print("[日志记录器] 警告: 日志列表为空，不保存文件。")
            return
        
        logged_data = np.array(self.joint_log)
        np.save(filename, logged_data)
        
        print(f"\n[日志记录器] Episode 数据已保存!")
        print(f"  - 文件: '{filename}'")
        print(f"  - 帧数: {logged_data.shape[0]}\n")
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        for i in range(self.num_leap_hand_dofs):
            ...
            leap_hand_dof_props['effort'][i] = 0.5  #这里很有可能要修改，因为机械臂的扭矩不一样
...
################################################################################################################
    ...
    def compute_observations(self):
        ...
        if self.cfg["env"]["include_history"]:
            ...
            self.obs_buf_lag_history[at_reset_env_ids, :, 0:16] = unscale(
                self.leap_hand_dof_pos[at_reset_env_ids, :16], 
                self.leap_hand_dof_lower_limits[at_reset_env_ids, :16],
                self.leap_hand_dof_upper_limits[at_reset_env_ids, :16]
            ).clone().unsqueeze(1)

            if self.cfg["env"]["include_targets"]:
                self.obs_buf_lag_history[at_reset_env_ids, :, 16:32] = self.leap_hand_dof_pos[at_reset_env_ids, :16].unsqueeze(1)
    ...
    def compute_reward(self, actions):
        # 定义关节索引 - 便于修改
        hand_dof_indices = list(range(16))  # 前16个是手指关节 (0-15)
        arm_dof_indices = [16, 17, 18, 19, 20]  # 后5个是机械臂关节
        
        # 定义不同关节的惩罚权重
        hand_pose_diff_scale = 1.0      # 手指关节的姿态差异惩罚权重
        arm_pose_diff_scale = 0.1       # 机械臂关节的姿态差异惩罚权重 (降低)
        hand_torque_scale = 1.0         # 手指关节的力矩惩罚权重
        arm_torque_scale = 0.3          # 机械臂关节的力矩惩罚权重 (降低)
        hand_work_scale = 1.0           # 手指关节的功率惩罚权重
        arm_work_scale = 0.1            # 机械臂关节的功率惩罚权重 (大幅降低)
        
        self.rot_axis_buf[:, -1] = -1
        
        # 分别计算手指关节和机械臂关节的惩罚
        # 手指关节姿态差异惩罚
        hand_pose_diff = ((self.leap_hand_dof_pos[:, hand_dof_indices] - 
                          self.init_pose_buf[:, hand_dof_indices]) ** 2).sum(-1)
        
        # 机械臂关节姿态差异惩罚
        arm_pose_diff = ((self.leap_hand_dof_pos[:, arm_dof_indices] - 
                         self.init_pose_buf[:, arm_dof_indices]) ** 2).sum(-1)
        
        # 加权合并姿态差异惩罚
        pose_diff_penalty = hand_pose_diff * hand_pose_diff_scale + arm_pose_diff * arm_pose_diff_scale
        
        # 手指关节力矩惩罚
        hand_torque_penalty = (self.torques[:, hand_dof_indices] ** 2).sum(-1)
        # 机械臂关节力矩惩罚
        arm_torque_penalty = (self.torques[:, arm_dof_indices] ** 2).sum(-1)
        # 加权合并力矩惩罚
        torque_penalty = hand_torque_penalty * hand_torque_scale + arm_torque_penalty * arm_torque_scale
        
        # 手指关节功率惩罚
        hand_work = (self.torques[:, hand_dof_indices] * self.dof_vel_finite_diff[:, hand_dof_indices]).sum(-1)
        # 机械臂关节功率惩罚
        arm_work = (self.torques[:, arm_dof_indices] * self.dof_vel_finite_diff[:, arm_dof_indices]).sum(-1)
        # 加权合并功率惩罚
        work_penalty = (hand_work * hand_work_scale + arm_work * arm_work_scale) ** 2
        ...
        # 添加分别的惩罚监控
        self.extras['hand_pose_diff_penalty'] = hand_pose_diff.mean()
        self.extras['arm_pose_diff_penalty'] = arm_pose_diff.mean()
        self.extras['hand_torque_penalty'] = hand_torque_penalty.mean()
        self.extras['arm_torque_penalty'] = arm_torque_penalty.mean()
        self.extras['hand_work_penalty'] = (hand_work ** 2).mean()
        self.extras['arm_work_penalty'] = (arm_work ** 2).mean()
    ...
    def pre_physics_step(self, actions):
        if hasattr(self, 'joint_log_enabled') and self.joint_log_enabled:
            if self.global_counter % self.log_interval == 0:
                current_joint_angles = self.leap_hand_dof_pos[0, :].clone().cpu().numpy()
                self.joint_log.append(current_joint_angles)

            if self.progress_buf[0] == self.max_episode_length - 1:
                if self.joint_log:
                    output_filename = os.path.join(
                        self.joint_log_dir, 
                        f"{self.joint_log_prefix}_{self.episode_log_counter}.npy"
                    )
                    self._save_joint_log(output_filename)
                    self.joint_log = []
