...
class LeapHandRot(VecTaskRot):
    def __init__(self, cfg, rl_device, sim_device, graphics_device_id, headless, virtual_screen_capture=None, force_render=None):
        ...
        if "actions_mask" in self.cfg["env"]:
            self.actions_mask = torch.tensor(self.cfg["env"]["actions_mask"], device=rl_device)[None, :]
        else:
            # 默认：前16个关节控制，后5个机械臂关节不控制
            mask = [1] * 16 + [0] * 5  # 假设总共21个关节
            self.actions_mask = torch.tensor(mask, device=rl_device)[None, :]
            
        # 🔧 添加：为被mask的关节设置合理的默认位置
        default_positions = [0.0] * 21  # 21个关节的默认位置
        # 为机械臂关节设置特定的默认位置（根据您的URDF调整）
        default_positions[16:] = [0.0, 0.0, 0.0, 0.0, 3.14]  # 机械臂关节默认位置
        self.leap_hand_default_dof_pos = torch.tensor(default_positions, dtype=torch.float, device=rl_device)
        
        super().__init__(cfg, rl_device, sim_device, graphics_device_id, headless)
        
        # 这些变量需要在super().__init__()之后设置，因为它们依赖于sim_params
        ...
        # 🔧 添加：更新默认位置设置以使用正确的DOF数量
        if hasattr(self, 'num_leap_hand_dofs'):
            default_positions = [0.0] * self.num_leap_hand_dofs 
            if self.num_leap_hand_dofs > 16:
                # 为机械臂关节设置特定的默认位置（根据您的URDF调整）
                default_positions[16:] = [0.0] * (self.num_leap_hand_dofs - 16)
            self.leap_hand_default_dof_pos = torch.tensor(default_positions, dtype=torch.float, device=self.device)
        ...
        if "debug" in self.cfg["env"]:
            ...
            if "actions_file" in self.cfg["env"]["debug"]:
                ...
                # 在__init__函数的最后添加
        print(f"=== Mask Configuration Debug ===")
        print(f"Total DOFs: {self.num_leap_hand_dofs}")
        print(f"Actions mask: {self.actions_mask}")
        print(f"Active joints: {self.actions_mask.sum().item()}")
        print(f"Masked joints: {(self.actions_mask == 0).sum().item()}")
        print(f"Default positions: {self.leap_hand_default_dof_pos}")
        print(f"================================")
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        for i in range(self.num_leap_hand_dofs):
            ...
            # 🔧 修改：根据mask设置不同的effort值
            if hasattr(self, 'actions_mask') and self.actions_mask[0, i] == 0:
                # 被mask的关节（机械臂关节）需要更大的effort来保持位置
                leap_hand_dof_props['effort'][i] = 2.0  # 机械臂关节需要更大扭矩
                leap_hand_dof_props['stiffness'][i] = self.cfg['env']['controller']['pgain'] * 2.0  # 增加刚度
                leap_hand_dof_props['damping'][i] = self.cfg['env']['controller']['dgain'] * 2.0    # 增加阻尼
            else:
                # 手部关节保持原始设置
...
################################################################################################################
        ...
        for n_s in range(num_scales):
            ...
            sampled_pose = self.saved_grasping_states[scale_key][sampled_pose_idx].clone()
                # 🔧 在这里添加缓存数据格式检查和处理逻辑
            if hasattr(self, 'actions_mask') and self.actions_mask.sum() == 16:
                # 使用16关节的缓存数据
                if sampled_pose.shape[1] == 23:  # 16 (joints) + 7 (object pose)
                    object_pose = sampled_pose[:, 16:]
                    joint_pose = sampled_pose[:, :16]
                elif sampled_pose.shape[1] == 28:  # 21 (joints) + 7 (object pose) - 向后兼容
                    print("Warning: Using 21-joint cache data with 16-joint mask")
                    object_pose = sampled_pose[:, 21:]
                    joint_pose = sampled_pose[:, :16]  # 只取前16个
                else:
                    raise ValueError(f"Unexpected cache data format: {sampled_pose.shape[1]}")
                
                # 设置物体位置
                self.root_state_tensor[self.object_indices[s_ids], :7] = object_pose
                self.root_state_tensor[self.object_indices[s_ids], 7:13] = 0
                
                # 为21个关节分配位置（后5个保持默认）
                full_pos = torch.zeros((len(s_ids), self.num_leap_hand_dofs), device=self.device)
                full_pos[:, :16] = joint_pose  # 前16个关节使用缓存数据
                # 后5个机械臂关节设置为默认位置
                if hasattr(self, 'leap_hand_default_dof_pos'):
                    full_pos[:, 16:] = self.leap_hand_default_dof_pos[16:].repeat(len(s_ids), 1)
                else:
                    # 如果没有默认位置，设置为0（需要根据您的URDF调整）
                    full_pos[:, 16:] = 0.0
                            # 🔧 添加：确保被mask的关节速度为0
                self.leap_hand_dof_vel[s_ids, :] = 0
                # 特别确保机械臂关节速度为0
                self.leap_hand_dof_vel[s_ids, 16:] = 0
                self.leap_hand_dof_pos[s_ids, :] = full_pos
                self.prev_targets[s_ids, :self.num_leap_hand_dofs] = full_pos
                self.cur_targets[s_ids, :self.num_leap_hand_dofs] = full_pos
                self.init_pose_buf[s_ids, :] = full_pos.clone()
                self.object_init_pose_buf[s_ids, :] = object_pose.clone()
            else:
                # 使用21关节的缓存数据（原始逻辑）
                assert sampled_pose.shape[1] == 28  # 21 (joints) + 7 (object pose)
                self.root_state_tensor[self.object_indices[s_ids], :7] = sampled_pose[:, 21:]
                self.root_state_tensor[self.object_indices[s_ids], 7:13] = 0
                pos = sampled_pose[:, :21]
                ...
                self.object_init_pose_buf[s_ids, :] = sampled_pose[:, 21:].clone()
    ...
    def compute_observations(self):
        ...
        if self.cfg["env"]["include_history"]:
            ...
            if hasattr(self, 'actions_mask') and self.actions_mask.sum() < self.num_leap_hand_dofs:
                # 只重置前16个关节的观测
                active_joints = int(self.actions_mask.sum().item())  # 16
                  # 🔧 添加：检查观测缓冲区大小是否足够
                required_size = active_joints * 2 if self.cfg["env"]["include_targets"] else active_joints
                if self.obs_buf_lag_history.shape[-1] < required_size:
                    print(f"Warning: obs_buf_lag_history size {self.obs_buf_lag_history.shape[-1]} < required {required_size}")
                
                    # 🔧 修正：只重置有效关节的观测
                self.obs_buf_lag_history[at_reset_env_ids, :, 0:active_joints] = unscale(
                    self.leap_hand_dof_pos[at_reset_env_ids, :active_joints], 
                    self.leap_hand_dof_lower_limits[at_reset_env_ids, :active_joints],
                    self.leap_hand_dof_upper_limits[at_reset_env_ids, :active_joints]
                ).clone().unsqueeze(1)

                if self.cfg["env"]["include_targets"]:
                    # 🔧 修正：目标观测应该从 active_joints 开始，而不是从 active_joints:active_joints*2
                    # 因为当前观测缓冲区的结构是 [joints(16) + targets(16) + other_features]
                    self.obs_buf_lag_history[at_reset_env_ids, :, active_joints:active_joints*2] = \
                        self.cur_targets[at_reset_env_ids, :active_joints].unsqueeze(1)
                        # 🔧 注意：这里应该用 cur_targets 而不是 leap_hand_dof_pos
            else:
                # 原始的21关节处理
                ...
                if self.cfg["env"]["include_targets"]:
                    self.obs_buf_lag_history[at_reset_env_ids, :, 21:42] = \
                        self.cur_targets[at_reset_env_ids].unsqueeze(1)
                            # 🔧 注意：这里也应该用 cur_targets
