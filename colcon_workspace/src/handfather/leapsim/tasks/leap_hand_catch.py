...
class LeapHandCATCH(VecTask):
    def __init__(self, cfg, rl_device, sim_device, graphics_device_id, headless, virtual_screen_capture=None, force_render=None):
        ...
        # 移除 save_init_pose 和 grasp_cache_name - 使用基于阶段的抓取策略
        ...
        # 移除缓存加载逻辑 - 现在使用基于阶段的抓取策略
        # 不再需要预计算的抓取姿态缓存
        
        # 基于阶段的抓取任务不需要旋转轴缓冲区
        # self.rot_axis_buf 已移除 - 专注于抓取而非旋转
        ...
        # 移除 previous_object_rot - 阶段抓取系统不需要跟踪物体旋转变化
        ...
        # 移除旋转奖励统计 - 改为抓取成功率统计
        # self.stat_sum_rotate_rewards = 0  # 不再需要旋转奖励
    ...
    def set_defaults(self):
        ...
        # 移除 grasp_dof_search_radius - 不再需要缓存搜索半径
        # 改为基于阶段的抓取策略默认配置
        ...
        if "include_obj_pose" not in self.cfg["env"]:
            self.cfg["env"]["include_obj_pose"] = True  # 抓取任务需要物体位姿信息
        ...
        # 移除旋转轴配置 - 基于阶段的抓取不需要旋转轴
        # rotation_axis 仅用于旋转任务，抓取任务专注于接近、接触、提升
    ...
    def reset_idx(self, env_ids):
        """
        重置指定环境的状态 - 使用基于阶段的抓取策略
        不再依赖预计算的抓取姿态缓存
        """
        # 随机化物体质量 - 对抓取任务很重要，影响抓取力度和稳定性
        ...
        # 移除无用的 else 分支 - 如果不随机化质量就不需要做任何操作
        ...
        # 重置基于阶段的抓取状态
        self.grasp_stage[env_ids] = 0  # 从接近阶段开始
    ...
    def _reset_object_poses(self, env_ids):
        """
        随机重置物体位置和方向 - 优化距离控制，确保物体在机械手可达范围内
        """
        num_resets = len(env_ids)
        
        # 在手部前方的合理可达范围内随机放置物体
        # 机械手位置: (0, 0, 0.5), 手心向下
        # x: 0.05 到 0.25 米（手部前方，避免过近和过远）
        # y: -0.15 到 0.15 米（左右摆动，减小范围提高成功率）
        # z: 0.3 到 0.5 米（手部下方到同一水平，确保可达）
        ...
        self.root_state_tensor[self.object_indices[env_ids], 7:13] = 0  # 速度设为零
