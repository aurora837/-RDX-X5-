...
class VecTask(Env):
    ...
    def __parse_sim_params(self, physics_engine: str, cfg_sim: Dict[str, Any]) -> gymapi.SimParams:
        """Parse the cfg dictionary for physics stepping settings.
        ...
        for nonphysical_param in ["observations", "actions"]:
            if nonphysical_param in dr_params and do_nonenv_randomize:
                ...
                if dist == 'gaussian':
                    ...
                    self.dr_randomizations[nonphysical_param] = {'mu': mu, 'var': var, 'mu_corr': mu_corr, 'var_corr': var_corr, 'noise_lambda': noise_lambda, 'wandb_log': dr_params[nonphysical_param].get('wandb_log', False)}
                    ...
                    self.dr_randomizations[nonphysical_param] = {'lo': lo, 'hi': hi, 'lo_corr': lo_corr, 'hi_corr': hi_corr, 'noise_lambda': noise_lambda, 'wandb_log': dr_params[nonphysical_param]['wandb_log']}

        if "exec_lag" in dr_params and do_nonenv_randomize:
            sched_step = dr_params["exec_lag"]["schedule_steps"] if "schedule_steps" in dr_params["exec_lag"] else None
            
            if sched_step is not None:
                if "threshold" not in dr_params["exec_lag"]:
                    sched_scaling = 1.0 / sched_step * min(self.last_step, sched_step)
                else:
                    if not hasattr(self, "exec_lag_num_thresholded_steps"):
                        self.exec_lag_num_thresholded_steps = 0
                    
                    sched_scaling = 1.0 / sched_step * min(self.exec_lag_num_thresholded_steps, sched_step)

                    if hasattr(self, "consecutive_successes"):
                        self.exec_lag_num_thresholded_steps += (self.consecutive_successes.item() >= dr_params["exec_lag"]["threshold"])
            else:
                sched_scaling = 1.0

            lo, hi = dr_params["exec_lag"]["range"]
            mean = lo + (hi - lo) * sched_scaling
            delta = dr_params["exec_lag"]["delta"] if "delta" in dr_params["exec_lag"] else 0
            delta = delta * sched_scaling

            def noise_lambda(tensor, param_name="exec_lag"):
                params = self.dr_randomizations[param_name]
                result = torch.ones_like(tensor) * params["mean"]
                result = result + (2 * torch.rand_like(result) - 1) * params["delta"]
                result = torch.clip(result, torch.ones_like(result) * params['lo'], torch.ones_like(result) * params['hi'])
                result = torch.round(result).long()

                return result

            self.dr_randomizations["exec_lag"] = {"lo": lo, "hi": hi, "mean": mean, "delta": delta, "noise_lambda": noise_lambda, 'wandb_log': dr_params["exec_lag"]['wandb_log']}        
